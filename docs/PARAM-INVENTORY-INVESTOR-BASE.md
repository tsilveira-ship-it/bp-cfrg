...
- Paramètres totaux (incl. métadonnées) : **300**
- Paramètres business (à auditer) : **300**

### Trous dans la raquette

- Sans source documentée : **212**
- Critiques/Hauts non validés 4-eyes : **65**
- Hors benchmark sectoriel : **0**
...
|----------|--------|---------|-----------|-------|---------|-------------|
| ARPU pondéré (€/mois TTC) | 141,25 | `Σ tier.monthlyPrice × tier.mixPct` | monthlyPriceCrossfit | 130–220 | in-range | Mix actuel: 15%/50%/20%/10%/5% |
| CAC implicite Y1 (€/membre) | 270 | `marketing.monthlyBudget × 12 / (rampEnd - rampStart)` | cacFitness | 80–150 | above | Hyp: 200 new members Y1 acquis pour 54 000€ |
| Membres fin d'horizon (estimation simple) | 553,89 | `rampEnd × Π (1 + growthRates[i])` | membersMatureCrossfit | 250–450 | above | Hors churn — borne haute du modèle. Box parisienne mature: 250-450. |
| Churn mensuel global (%/mo) | 2.00% | `subs.monthlyChurnPct` | churnCrossfitCommunity | 0.015–0.03 | in-range |  |
...
| `bfr.daysOfRevenue` | bfr | 15 | high | Encaissement Square + délais fournisseurs | to-validate | — | n/a | none | — |
...
| `financing.bonds[0].annualRatePct` | financing.bonds | 8 | high | Marché obligations PME | to-validate | — | n/a | none | — |
...
| `financing.equity[1].amount` | financing.equity | 400 000 | critical | Engagements fonds propres | to-validate | — | n/a | none | — |
...
| `financing.equity[2].amount` | financing.equity | 150 000 | critical | Engagements fonds propres | to-validate | — | n/a | none | — |
| `financing.equity[2].id` | financing.equity | tranche_b | unknown | Non documenté | unmapped | — | n/a | none | — |
| `financing.equity[2].name` | financing.equity | Tranche B (refinancement bond) | unknown | Non documenté | unmapped | — | n/a | none | — |
| `financing.equity[2].startMonth` | financing.equity | 48 | unknown | Non documenté | unmapped | — | n/a | none | — |
| `financing.loans[0].annualRatePct` | financing.loans | 4 | high | Indication banque | to-validate | — | n/a | none | Tornado loanRate ±30% |
...
| `marketing.monthlyBudget` | costs.marketing | 4 500 | critical | Budget acquisition arbitré | to-validate | — | n/a | none | Tornado ±20%; Decompose par canal |
| `marketing.pctOfRevenue` | costs.marketing | 2.00% | high | Décision | to-validate | — | n/a | none | — |
...
| `openingCash` | cash | 50 000 | high | Trésorerie M0 hors financements | to-validate | — | n/a | none | — |
...
| `recurring[2].monthly` | costs.recurring | 2 500 | medium | Devis / contrats fournisseurs | to-validate | — | n/a | none | — |
...
| `recurring[4].monthly` | costs.recurring | 0 | medium | Devis / contrats fournisseurs | to-validate | — | n/a | none | — |
...
| `rent.monthlyByFy` | costs.rent | [10000, 13000, 12500, 12500, 12500, 12500, 12500] | critical | Bail commercial / LOI bailleur | missing | rentPerSqmParisYear: 350–700 | n/a | none | Tornado rent ±10%; Sensibilité ratio Y1→Y2 |
...
| `salaries.annualIndexPa` | costs.salaries | 2.00% | high | SMIC + convention collective | to-validate | — | n/a | none | AUDIT-CORRECTED utilise 2% |
...
| `subs.growthRates` | revenue.subscriptions | [0.3, 0.22, 0.15, 0.08, 0.05, 0.03] | critical | Trajectoire ramp post-FY26 | to-validate | — | n/a | none | Tornado subsGrowth; Décomposer acquisition vs churn |
| `subs.monthlyChurnPct` | revenue.subscriptions | 2.00% | critical | À sourcer cohorte réelle | missing | churnCrossfitCommunity: 0.015–0.03 | in-range | none | Tornado churn 0→3%/mo; Reverse: churn max avant cashTrough<0 |
| `subs.priceIndexPa` | revenue.subscriptions | 2.00% | high | Décision pricing — inflation FR ~2% | to-validate | — | n/a | none | AUDIT-CORRECTED utilise 2% |
| `subs.rampEndCount` | revenue.subscriptions | 260 | critical | Cible interne fin FY26 | to-validate | — | n/a | none | Tornado ±20%; Calibrate vs Reebok Louvre Y1 réel |
| `subs.rampStartCount` | revenue.subscriptions | 60 | critical | Hypothèse pré-inscriptions M0 | missing | — | n/a | none | Reverse: ramp min pour break-even <24mo |
...
| `subs.tiers[0].mixPct` | revenue.subscriptions | 15.00% | critical | Hypothèse fondateur basée sur historique legacy | to-validate | — | n/a | none | Tornado mixPremiumShift; Sensibilité ARPU pondéré |
| `subs.tiers[0].monthlyPrice` | revenue.subscriptions | 180 | critical | Tarifs catalogue CFRG (5 tiers abonnement) | to-validate | — | n/a | none | Tornado ±10%; Reverse: prix min pour cashTrough>0 |
...
| `subs.tiers[1].mixPct` | revenue.subscriptions | 50.00% | critical | Hypothèse fondateur basée sur historique legacy | to-validate | — | n/a | none | Tornado mixPremiumShift; Sensibilité ARPU pondéré |
| `subs.tiers[1].monthlyPrice` | revenue.subscriptions | 150 | critical | Tarifs catalogue CFRG (5 tiers abonnement) | to-validate | — | n/a | none | Tornado ±10%; Reverse: prix min pour cashTrough>0 |
...
| `subs.tiers[2].monthlyPrice` | revenue.subscriptions | 125 | critical | Tarifs catalogue CFRG (5 tiers abonnement) | to-validate | — | n/a | none | Tornado ±10%; Reverse: prix min pour cashTrough>0 |
...
| `subs.tiers[3].mixPct` | revenue.subscriptions | 10.00% | critical | Hypothèse fondateur basée sur historique legacy | to-validate | — | n/a | none | Tornado mixPremiumShift; Sensibilité ARPU pondéré |
| `subs.tiers[3].monthlyPrice` | revenue.subscriptions | 105 | critical | Tarifs catalogue CFRG (5 tiers abonnement) | to-validate | — | n/a | none | Tornado ±10%; Reverse: prix min pour cashTrough>0 |
...
| `subs.tiers[4].mixPct` | revenue.subscriptions | 5.00% | critical | Hypothèse fondateur basée sur historique legacy | to-validate | — | n/a | none | Tornado mixPremiumShift; Sensibilité ARPU pondéré |
| `subs.tiers[4].monthlyPrice` | revenue.subscriptions | 75 | critical | Tarifs catalogue CFRG (5 tiers abonnement) | to-validate | — | n/a | none | Tornado ±10%; Reverse: prix min pour cashTrough>0 |
...
| `tax.enableDA` | tax | true | critical | Décision modèle | to-validate | — | n/a | none | — |
| `tax.enableIs` | tax | true | critical | Décision modèle | to-validate | — | n/a | none | — |
| `tax.enableLossCarryForward` | tax | true | unknown | Non documenté | unmapped | — | n/a | none | — |
...
- `bfr.daysOfRevenue` = 15 — 0j dans DEFAULT_PARAMS = irréaliste. AUDIT-CORRECTED: 15j.
...
- `financing.bonds[0].annualRatePct` = 8 — 6% PME pré-CA = bas. 8-10% plus réaliste.
...
- `financing.equity[1].amount` = 400 000 — 400k€ equity dont 100k perso. À matérialiser (term sheet, virement).
- `financing.equity[2].amount` = 150 000 — 400k€ equity dont 100k perso. À matérialiser (term sheet, virement).
- `financing.loans[0].annualRatePct` = 4 — 3% taux 2026 = optimiste sur BCE. 4-5% plus défendable.
...
- `marketing.monthlyBudget` = 4 500 — Budget marketing mensuel fixe. CAC implicite = budget Y1 / new members Y1.
- `marketing.pctOfRevenue` = 2.00% — Mix fixe vs %CA. AUDIT-CORRECTED utilise 2% CA.
- `openingCash` = 50 000 — 0 = financement couvre 100% du démarrage. Réserve fondateur ?
- `rent.monthlyByFy` = [10000, 13000, 12500, 12500, 12500, 12500, 12500] — Saut FY26→FY27 (10000→18113) atypique. LOI/bail requis.
- `salaries.annualIndexPa` = 2.00% — SMIC indexé chaque année. 0% interdit légalement, +2% défendable.
...
- `subs.growthRates` = [0.3, 0.22, 0.15, 0.08, 0.05, 0.03] — Croissance YoY combinée à churn doit rester cohérente (pas magic growth).
- `subs.monthlyChurnPct` = 2.00% — Churn=0 = kill VC. Cible défendable: 1.5-3%/mo.
- `subs.priceIndexPa` = 2.00% — Geler prix nominal = baisse prix réel ~15% horizon 7y. À justifier.
- `subs.rampEndCount` = 260 — 200 fin M12 = ramp Y1 ambitieux. Boxes parisiennes atteignent 250-450 en 18-36 mo, pas 12.
- `subs.rampStartCount` = 60 — Effectif M0. 80 membres dès ouverture = signal demande contractualisé requis (LOI, dépôts pré-inscription).
- `subs.tiers[0].mixPct` = 15.00% — Mix tier détermine ARPU. Bascule premium → cheap = -15-20% revenu.
- `subs.tiers[0].monthlyPrice` = 180 — Prix de chaque tier. Le contrôle benchmark se fait sur ARPU pondéré (cf agrégats).
- `subs.tiers[1].mixPct` = 50.00% — Mix tier détermine ARPU. Bascule premium → cheap = -15-20% revenu.
- `subs.tiers[1].monthlyPrice` = 150 — Prix de chaque tier. Le contrôle benchmark se fait sur ARPU pondéré (cf agrégats).
- `subs.tiers[2].mixPct` = 20.00% — Mix tier détermine ARPU. Bascule premium → cheap = -15-20% revenu.
- `subs.tiers[2].monthlyPrice` = 125 — Prix de chaque tier. Le contrôle benchmark se fait sur ARPU pondéré (cf agrégats).
- `subs.tiers[3].mixPct` = 10.00% — Mix tier détermine ARPU. Bascule premium → cheap = -15-20% revenu.
- `subs.tiers[3].monthlyPrice` = 105 — Prix de chaque tier. Le contrôle benchmark se fait sur ARPU pondéré (cf agrégats).
- `subs.tiers[4].mixPct` = 5.00% — Mix tier détermine ARPU. Bascule premium → cheap = -15-20% revenu.
- `subs.tiers[4].monthlyPrice` = 75 — Prix de chaque tier. Le contrôle benchmark se fait sur ARPU pondéré (cf agrégats).
- `subs.vatRate` = 20.00% — TVA 20% standard prestations sportives non éligibles taux réduit.
- `tax.enableDA` = true — false = pas d'amortissement = P&L surévalué.
- `tax.enableIs` = true — false dans DEFAULT_PARAMS = bug audit. AUDIT-CORRECTED active.
...
- `financing.equity[2].id` = tranche_b (catégorie financing.equity, criticité unknown)
- `financing.equity[2].name` = Tranche B (refinancement bond) (catégorie financing.equity, criticité unknown)
- `financing.equity[2].startMonth` = 48 (catégorie financing.equity, criticité unknown)
...
- `rent.monthlyByFy` = [10000, 13000, 12500, 12500, 12500, 12500, 12500] (catégorie costs.rent, criticité critical)
...
- `subs.monthlyChurnPct` = 2.00% (catégorie revenue.subscriptions, criticité critical)
- `subs.rampStartCount` = 60 (catégorie revenue.subscriptions, criticité critical)
...
- `tax.enableLossCarryForward` = true (catégorie tax, criticité unknown)
