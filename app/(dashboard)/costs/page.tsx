    ...
    <div className="space-y-6">
      <header className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Charges opérationnelles par poste, FY25 → FY29</p>
        </div>
        <ScenarioSwitcher />
      </header>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-right">% CA FY29</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {[
                ...
                <TableRow key={row.k}>
                  ...
                  {result.yearly.map((y, i) => (
                    <TableCell key={i} className="text-right">{fmtCurrency(y[row.k], { compact: true })}</TableCell>
                  ))}
                  <TableCell className="text-right text-xs">
                    {fmtPct(result.yearly[4][row.k] / Math.max(1, result.yearly[4].totalRevenue))}
                  </TableCell>
                </TableRow>
              ))}
              <TableRow className="font-bold border-t-2 bg-muted/30">
                ...
                {result.yearly.map((y, i) => (
                  <TableCell key={i} className="text-right">{fmtCurrency(y.totalOpex, { compact: true })}</TableCell>
                ))}
                <TableCell className="text-right">{fmtPct(result.yearly[4].totalOpex / result.yearly[4].totalRevenue)}</TableCell>
              </TableRow>
              <TableRow className="font-bold bg-emerald-50/40">
                ...
                <TableCell className="text-right">{fmtPct(result.yearly[4].ebitdaMargin)}</TableCell>
              </TableRow>
            </TableBody>
          </Table>
        </CardContent>
