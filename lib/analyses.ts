...
export const ANALYSES: Record<string, string> = {
  ...
  "Acquisitions par FY": "1 valeur d'acquisitions brutes/mois par année (FY). Constante intra-FY, modulée uniquement par la saisonnalité mensuelle (Sept ×1.20 / Août ×0.60). Remplace l'ancien shape acquisitionStart→End ramp + acquisitionGrowthByFy en mode cohort — plus simple à calibrer et à défendre devant un investisseur.",
  "Ramp-up": "Concept legacy : interpolation linéaire entre membres début et fin de FY0 (puis croissance FY-over-FY). Remplacé par 'Acquisitions par FY' en mode cohort. Conservé pour rétro-compat des scénarios anciens et pour le bouton 'Seed depuis cible NET' qui calcule des acquisitions/mois plates à partir d'une cible NET via Little's law.",
  ...
  "Canaux d'acquisition": "[Déprécié] Concept remplacé par le 'Funnel commercial' concret (leads → appels → bilans → abos + coût horaire freelance + budget ads). Voir /funnel-bp.",
  "Funnel commercial": "Modèle pivot : on part de la cible d'acquisitions/mois (saisie dans 'Acquisitions par FY' du mode cohort), on back-calcule les leads nécessaires via le ratio 'Leads / acquisition', puis on dérive appels freelance + heures × taux horaire + bonus + ads. Une partie des acquisitions passe par bilan, l'autre signe directement. Remplace 'marketing.monthlyBudget' flat dans le P&L. Voir /funnel-bp.",
  "Leads par acquisition": "Ratio inverse de la conversion globale : combien de leads bruts faut-il pour signer 1 abo. Ex : 12 leads/abo = 8,3% conv globale. Combine en un seul nombre les sous-conversions (appel, bilan, signature). Plus simple à calibrer que 3 % séparés.",
  "% via bilan": "Part des acquisitions qui passent par une séance Bilan 19,90€ (le reste signe directement). 60% typique. Le bilan génère un revenu HT qui recouvre partiellement le coût marketing.",
  "Lead funnel": "Voir 'Funnel commercial'. Modèle back-calc : acquisitions cible → leads → appels → coûts.",
  "Heures freelance": "Calcul automatique : appels effectifs × minutes par appel / 60. Permet de visualiser l'équivalent FTE freelance nécessaire (137h productives/mois pour 1 FTE).",
  "CAC effectif funnel": "Coût acquisition réel calculé par le funnel commercial : (total coût marketing − revenu bilan HT) ÷ acquisitions. Plus précis que le CAC implicite marketing/acquisitions car intègre le crédit revenu bilan (le bilan à 19,90€ recouvre partiellement le coût freelance).",
